...
pub use self::ast_h::{
    ast_child, ast_free_unattached, ast_get_children, ast_id, ast_ptr_t, ast_sibling,
};
use self::cap_h::is_cap_compat_cap;
...
use self::viewpoint_h::{viewpoint_lower, viewpoint_reifypair, viewpoint_reifytypeparam};
#[c2rust::src_loc = "6:1"]
unsafe extern "C" fn is_nominal_compat_nominal(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
...
unsafe extern "C" fn is_nominal_compat_typeparam(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    ...
    let mut children_0: [*mut *mut ast_t; 4] =
        [&mut b_id, &mut b_cap, &mut b_eph, 0 as *mut *mut ast_t];
    ast_get_children(
        b,
...
unsafe extern "C" fn is_typeparam_compat_typeparam(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    ...
    let mut children: [*mut *mut ast_t; 4] =
        [&mut a_id, &mut a_cap, &mut a_eph, 0 as *mut *mut ast_t];
    ast_get_children(
        a,
        (::core::mem::size_of::<[*mut *mut ast_t; 4]>() as libc::c_ulong)
            .wrapping_div(::core::mem::size_of::<*mut *mut ast_t>() as libc::c_ulong)
...
#[c2rust::src_loc = "42:1"]
unsafe extern "C" fn is_arrow_compat_nominal(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    let mut a_lower: *mut ast_t = viewpoint_lower(a);
    if a.is_null() {
...
unsafe extern "C" fn is_arrow_compat_typeparam(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    let mut r_a: *mut ast_t = viewpoint_reifytypeparam(a, b);
    let mut r_b: *mut ast_t = viewpoint_reifytypeparam(b, b);
    if !r_a.is_null() {
        let mut ok: bool = is_compat_type(r_a, r_b);
...
#[c2rust::src_loc = "79:1"]
unsafe extern "C" fn is_arrow_compat_arrow(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    let mut r_a: *mut ast_t = 0 as *mut ast_t;
    let mut r_b: *mut ast_t = 0 as *mut ast_t;
    if viewpoint_reifypair(a, b, &mut r_a, &mut r_b) {
        let mut ok: bool = is_compat_type(r_a, r_b);
        ast_free_unattached(r_a);
        ast_free_unattached(r_b);
        return ok;
...
unsafe extern "C" fn is_union_compat_x(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    let mut child: *mut ast_t = ast_child(a);
    while !child.is_null() {
...
unsafe extern "C" fn is_isect_compat_x(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    let mut child: *mut ast_t = ast_child(a);
    while !child.is_null() {
        if !is_compat_type(child, b) {
            return 0 as libc::c_int != 0;
...
unsafe extern "C" fn is_tuple_compat_tuple(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    let mut a_child: *mut ast_t = ast_child(a);
    let mut b_child: *mut ast_t = ast_child(b);
    ...
    return a_child.is_null() && b_child.is_null();
}
#[c2rust::src_loc = "178:1"]
unsafe extern "C" fn is_tuple_compat_x(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    match ast_id(b) as libc::c_uint {
        149 => return is_union_compat_x(b, a),
        56 => return is_isect_compat_x(b, a),
        150 => return is_tuple_compat_tuple(b, a),
        151 => return 0 as libc::c_int != 0,
        187 => return 0 as libc::c_int != 0,
        17 => return 0 as libc::c_int != 0,
        _ => {}
    }
...
unsafe extern "C" fn is_nominal_compat_x(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    match ast_id(b) as libc::c_uint {
        149 => return is_union_compat_x(b, a),
        56 => return is_isect_compat_x(b, a),
        150 => return is_tuple_compat_x(b, a),
        151 => return is_nominal_compat_nominal(a, b),
        187 => return is_nominal_compat_typeparam(a, b),
        17 => return is_arrow_compat_nominal(b, a),
        _ => {}
    }
    if 0 as libc::c_int != 0 {
    } else {
...
unsafe extern "C" fn is_typeparam_compat_x(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    match ast_id(b) as libc::c_uint {
        149 => return is_union_compat_x(b, a),
        56 => return is_isect_compat_x(b, a),
        150 => return is_tuple_compat_x(b, a),
        151 => return is_nominal_compat_typeparam(b, a),
        187 => return is_typeparam_compat_typeparam(a, b),
        17 => return is_arrow_compat_typeparam(b, a),
        _ => {}
    }
    if 0 as libc::c_int != 0 {
    } else {
...
unsafe extern "C" fn is_arrow_compat_x(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    match ast_id(b) as libc::c_uint {
        149 => return is_union_compat_x(b, a),
        56 => return is_isect_compat_x(b, a),
        150 => return is_tuple_compat_x(b, a),
        151 => return is_arrow_compat_nominal(a, b),
        187 => return is_arrow_compat_typeparam(a, b),
        17 => return is_arrow_compat_arrow(a, b),
        _ => {}
    }
    if 0 as libc::c_int != 0 {
    } else {
...
pub unsafe extern "C" fn is_compat_type(mut a: *mut ast_t, mut b: *mut ast_t) -> bool {
    match ast_id(a) as libc::c_uint {
        149 => return is_union_compat_x(a, b),
        56 => return is_isect_compat_x(a, b),
        150 => return is_tuple_compat_x(a, b),
        151 => return is_nominal_compat_x(a, b),
        187 => return is_typeparam_compat_x(a, b),
        17 => return is_arrow_compat_x(a, b),
        _ => {}
    }
    if 0 as libc::c_int != 0 {
    } else {
