...
#[c2rust::src_loc = "9:1"]
unsafe extern "C" fn entity_serialiser(
    mut opt: *mut pass_opt_t,
    mut entity: *mut ast_t,
    mut serialise_space: *const libc::c_char,
    mut serialise: *const libc::c_char,
    mut deserialise: *const libc::c_char,
) -> bool {
    let mut ast_serialise_space: *mut ast_t =
        ast_get(entity, serialise_space, 0 as *mut sym_status_t);
    let mut ast_serialise: *mut ast_t = ast_get(entity, serialise, 0 as *mut sym_status_t);
    let mut ast_deserialise: *mut ast_t = ast_get(entity, deserialise, 0 as *mut sym_status_t);
    if ast_serialise_space.is_null() && ast_serialise.is_null() && ast_deserialise.is_null() {
    ...
    if ast_serialise_space.is_null() || ast_serialise.is_null() || ast_deserialise.is_null() {
        ast_error(
            (*opt).check.errors,
            entity,
            b"If a custom serialisation strategy exists then _serialise_space, _serialise and _deserialise must all be provided\0"
...
#[c2rust::src_loc = "32:1"]
unsafe extern "C" fn module_serialisers(
    mut opt: *mut pass_opt_t,
    ...
    while !entity.is_null() {
        match ast_id(entity) as libc::c_uint {
            76 => {
                if !entity_serialiser(opt, entity, serialise_space, serialise, deserialise) {
...
unsafe extern "C" fn package_serialisers(
    ...
    while !module.is_null() {
        if !module_serialisers(opt, module, serialise_space, serialise, deserialise) {
            ok = 0 as libc::c_int != 0;
        }
...
#[no_mangle]
#[c2rust::src_loc = "75:1"]
pub unsafe extern "C" fn pass_serialisers(
    mut program: *mut ast_t,
    mut options: *mut pass_opt_t,
) -> bool {
    let mut package: *mut ast_t = ast_child(program);
    let mut serialise_space: *const libc::c_char =
        stringtab(b"_serialise_space\0" as *const u8 as *const libc::c_char);
    let mut serialise: *const libc::c_char =
        stringtab(b"_serialise\0" as *const u8 as *const libc::c_char);
    let mut deserialise: *const libc::c_char =
        stringtab(b"_deserialise\0" as *const u8 as *const libc::c_char);
    let mut ok: bool = 1 as libc::c_int != 0;
    while !package.is_null() {
        if !package_serialisers(options, package, serialise_space, serialise, deserialise) {
            ok = 0 as libc::c_int != 0;
        }
