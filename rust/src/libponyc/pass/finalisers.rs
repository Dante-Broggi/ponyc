...
pub mod ast_h {
    ...
    extern "C" {
        ...
        #[c2rust::src_loc = "82:1"]
        pub fn ast_cansend(ast: *mut ast_t) -> bool;
        #[c2rust::src_loc = "83:1"]
        pub fn ast_setsend(ast: *mut ast_t);
        #[c2rust::src_loc = "84:1"]
        pub fn ast_mightsend(ast: *mut ast_t) -> bool;
        #[c2rust::src_loc = "85:1"]
...
pub use self::ast_h::{
    ...
    ast_name, ast_ptr_t, ast_setflag, ast_setmightsend, ast_setsend, ast_sibling, ast_type,
...
#[c2rust::src_loc = "12:3"]
pub const FINAL_MAY_SEND: C2RustUnnamed_0 = 1;
#[c2rust::src_loc = "11:3"]
pub const FINAL_NO_SEND: C2RustUnnamed_0 = 0;
#[c2rust::src_loc = "13:3"]
pub const FINAL_CAN_SEND: C2RustUnnamed_0 = 2;
#[c2rust::src_loc = "14:3"]
pub const FINAL_RECURSE: C2RustUnnamed_0 = 4;
#[c2rust::src_loc = "9:1"]
pub type C2RustUnnamed_0 = libc::c_uint;
#[c2rust::src_loc = "19:1"]
unsafe extern "C" fn show_send(mut opt: *mut pass_opt_t, mut ast: *mut ast_t) {
    let mut child: *mut ast_t = ast_child(ast);
    while !child.is_null() {
        if ast_cansend(child) as libc::c_int != 0 || ast_mightsend(child) as libc::c_int != 0 {
            show_send(opt, child);
        }
    ...
    if ast_id(ast) as libc::c_uint == TK_CALL as libc::c_int as libc::c_uint {
        if ast_cansend(ast) {
            ast_error_continue(
                (*opt).check.errors,
                ast,
                b"a message can be sent here\0" as *const u8 as *const libc::c_char,
            );
        } else if ast_mightsend(ast) {
            ast_error_continue(
                (*opt).check.errors,
                ast,
                b"a message might be sent here\0" as *const u8 as *const libc::c_char,
            );
        }
    }
}
#[c2rust::src_loc = "41:1"]
unsafe extern "C" fn receiver_def(mut type_0: *mut ast_t) -> *mut ast_t {
    match ast_id(type_0) as libc::c_uint {
        56 => {
            let mut child: *mut ast_t = ast_child(type_0);
            while !child.is_null() {
                let mut def: *mut ast_t = receiver_def(child);
                if !def.is_null() {
                    match ast_id(def) as libc::c_uint {
                        74 | 75 | 76 | 77 => return def,
                        _ => {}
                    }
                }
                child = ast_sibling(child);
        ...
        151 => return ast_data(type_0) as *mut ast_t,
        17 => return receiver_def(ast_childidx(type_0, 1 as libc::c_int as size_t)),
        187 => {
            let mut def_0: *mut ast_t = ast_data(type_0) as *mut ast_t;
            return receiver_def(ast_childidx(def_0, 1 as libc::c_int as size_t));
        }
        _ => {}
    }
...
#[c2rust::src_loc = "96:1"]
unsafe extern "C" fn check_call_send(mut ast: *mut ast_t, mut in_final: bool) -> libc::c_int {
    ...
    if !is_known(type_0) {
        return FINAL_MAY_SEND as libc::c_int;
    }
    let mut def: *mut ast_t = receiver_def(type_0);
    if !def.is_null() {
    } else {
        ponyint_assert_fail(
            b"def != NULL\0" as *const u8 as *const libc::c_char,
            b"/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/pass/finalisers.c\0"
                as *const u8 as *const libc::c_char,
            122 as libc::c_int as size_t,
            (*::core::mem::transmute::<&[u8; 16], &[libc::c_char; 16]>(b"check_call_send\0"))
                .as_ptr(),
        );
    };
    let mut method_name: *const libc::c_char = ast_name(method);
    let mut fun: *mut ast_t = ast_get(def, method_name, 0 as *mut sym_status_t);
    if !fun.is_null() {
    } else {
        ponyint_assert_fail(
    ...
    } else if in_final as libc::c_int != 0 && r == FINAL_RECURSE as libc::c_int {
        ast_clearmightsend(ast);
    } else if r & FINAL_CAN_SEND as libc::c_int != 0 as libc::c_int {
        ast_setsend(ast);
    } else if r & FINAL_MAY_SEND as libc::c_int != 0 as libc::c_int {
        ast_setmightsend(ast);
    }
    return r;
}
#[c2rust::src_loc = "150:1"]
unsafe extern "C" fn check_expr_send(mut ast: *mut ast_t, mut in_final: bool) -> libc::c_int {
    let mut send: libc::c_int = FINAL_NO_SEND as libc::c_int;
    if ast_id(ast) as libc::c_uint == TK_CALL as libc::c_int as libc::c_uint {
...
unsafe extern "C" fn check_body_send(mut ast: *mut ast_t, mut in_final: bool) -> libc::c_int {
    ...
    if ast_cansend(ast) {
        return FINAL_CAN_SEND as libc::c_int;
    }
...
unsafe extern "C" fn entity_finaliser(
    mut opt: *mut pass_opt_t,
    mut entity: *mut ast_t,
    mut final_0: *const libc::c_char,
) -> bool {
    let mut ast: *mut ast_t = ast_get(entity, final_0, 0 as *mut sym_status_t);
    if ast.is_null() {
    ...
    if r & FINAL_CAN_SEND as libc::c_int != 0 as libc::c_int
        || r & FINAL_MAY_SEND as libc::c_int != 0 as libc::c_int
    {
        ast_error(
            (*opt).check.errors,
            ast,
            b"_final cannot create actors or send messages\0" as *const u8 as *const libc::c_char,
        );
        show_send(opt, body);
...
unsafe extern "C" fn module_finalisers(
    mut opt: *mut pass_opt_t,
    ...
    while !entity.is_null() {
        match ast_id(entity) as libc::c_uint {
            77 | 76 | 75 | 74 => {
                if !entity_finaliser(opt, entity, final_0) {
...
unsafe extern "C" fn package_finalisers(
    ...
    while !module.is_null() {
        if !module_finalisers(opt, module, final_0) {
            ok = 0 as libc::c_int != 0;
        }
