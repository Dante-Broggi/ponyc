...
unsafe extern "C" fn tuple_access(mut opt: *mut pass_opt_t, mut ast: *mut ast_t) -> bool {
    ...
    let mut right_idx: size_t = (*ast_int(right)).low as size_t;
    let mut tuple_size: size_t = ast_childcount(type_0);
    if right_idx == -(1 as libc::c_int) as size_t {
        ast_error(
            (*opt).check.errors,
            right,
