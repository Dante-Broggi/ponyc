...
#[c2rust::header_src = "/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/pkg/ifdef.h:4"]
pub mod ifdef_h {
    use super::frame_h::typecheck_t;
    use super::pass_h::pass_opt_t;
    use super::symtab_h::ast_t;
...
#[c2rust::header_src = "/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/type/alias.h:6"]
pub mod alias_h {
    use super::symtab_h::ast_t;
    extern "C" {
        #[c2rust::src_loc = "11:1"]
...
pub use self::ast_h::{
    ast_child, ast_childcount, ast_childidx, ast_error, ast_error_frame, ast_free_unattached,
    ast_get_children, ast_id, ast_name, ast_print_type, ast_ptr_t, ast_setdata, ast_settype,
...
use self::ifdef_h::ffi_get_decl;
use self::literal_h::{coerce_literals, is_type_literal};
pub use self::pass_h::{
...
#[no_mangle]
#[c2rust::src_loc = "10:1"]
pub unsafe extern "C" fn void_star_param(
    mut param_type: *mut ast_t,
    mut arg_type: *mut ast_t,
) -> bool {
    if !param_type.is_null() {
    } else {
        ponyint_assert_fail(
            b"param_type != NULL\0" as *const u8 as *const libc::c_char,
            b"/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/expr/ffi.c\0" as *const u8
                as *const libc::c_char,
            12 as libc::c_int as size_t,
            (*::core::mem::transmute::<&[u8; 16], &[libc::c_char; 16]>(b"void_star_param\0"))
                .as_ptr(),
        );
    };
    if !arg_type.is_null() {
        ...
        ponyint_assert_fail(
            b"arg_type != NULL\0" as *const u8 as *const libc::c_char,
            b"/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/expr/ffi.c\0" as *const u8
                as *const libc::c_char,
    ...
    let mut type_args: *mut ast_t = ast_childidx(param_type, 2 as libc::c_int as size_t);
    if ast_childcount(type_args) != 1 as libc::c_int as libc::c_ulong
        || !is_none(ast_child(type_args))
    {
        return 0 as libc::c_int != 0;
    }
    while ast_id(arg_type) as libc::c_uint == TK_ARROW as libc::c_int as libc::c_uint {
        arg_type = ast_childidx(arg_type, 1 as libc::c_int as size_t);
    }
    if is_pointer(arg_type) as libc::c_int != 0
        || is_nullable_pointer(arg_type) as libc::c_int != 0
        || is_literal(arg_type, b"USize\0" as *const u8 as *const libc::c_char) as libc::c_int != 0
    {
...
#[c2rust::src_loc = "36:1"]
unsafe extern "C" fn declared_ffi(
    ...
    if !call.is_null() {
        ...
        ponyint_assert_fail(
            ...
            (*::core::mem::transmute::<&[u8; 13], &[libc::c_char; 13]>(b"declared_ffi\0")).as_ptr(),
        );
    };
    if !decl.is_null() {
    } else {
    ...
    while !arg.is_null()
        && !param.is_null()
        && ast_id(param) as libc::c_uint != TK_ELLIPSIS as libc::c_int as libc::c_uint
    {
        ...
        if ast_id(arg_type) as libc::c_uint == TK_TUPLETYPE as libc::c_int as libc::c_uint {
            ast_error(
                (*opt).check.errors,
                arg,
                b"cannot pass tuples as FFI arguments\0" as *const u8 as *const libc::c_char,
            );
            return 0 as libc::c_int != 0;
        }
        let mut a_type: *mut ast_t = alias(arg_type);
        let mut info: errorframe_t = 0 as errorframe_t;
        if !void_star_param(p_type, a_type) && !is_subtype(a_type, p_type, &mut info, opt) {
            let mut frame: errorframe_t = 0 as errorframe_t;
            ast_error_frame(
                &mut frame as *mut errorframe_t,
                arg,
                b"argument not a assignable to parameter\0" as *const u8 as *const libc::c_char,
            );
            ast_error_frame(
                &mut frame as *mut errorframe_t,
                arg,
    ...
    while !arg.is_null() {
        let mut a_type_0: *mut ast_t = ast_type(arg);
        if !a_type_0.is_null() && is_type_literal(a_type_0) as libc::c_int != 0 {
            ast_error(
                (*opt).check.errors,
                arg,
                b"Cannot pass number literals as unchecked FFI arguments\0" as *const u8
                    as *const libc::c_char,
            );
            return 0 as libc::c_int != 0;
        }
    ...
    let mut call_ret_type: *mut ast_t = ast_child(call_ret_typeargs);
    let mut decl_ret_type: *mut ast_t = ast_child(decl_ret_typeargs);
    let mut f_name: *const libc::c_char = (ast_name(decl_name)).offset(1 as libc::c_int as isize);
    let mut intrinsic: bool = strncmp(
        f_name,
    ...
    if !intrinsic
        && ast_id(decl_ret_type) as libc::c_uint == TK_TUPLETYPE as libc::c_int as libc::c_uint
    {
        ast_error(
            (*opt).check.errors,
            decl_ret_type,
            b"an FFI function cannot return a tuple\0" as *const u8 as *const libc::c_char,
        );
        return 0 as libc::c_int != 0;
    }
    if !intrinsic
        && !call_ret_type.is_null()
        && ast_id(call_ret_type) as libc::c_uint == TK_TUPLETYPE as libc::c_int as libc::c_uint
    {
        ast_error(
            (*opt).check.errors,
            call_ret_type,
    ...
    let mut ret_type: *mut ast_t = 0 as *mut ast_t;
    if !call_ret_type.is_null() {
        ret_type = call_ret_type;
    } else {
        ret_type = decl_ret_type;
    }
    ast_setdata(call, decl as *mut libc::c_void);
    ast_settype(call, ret_type);
    return 1 as libc::c_int != 0;
}
#[no_mangle]
#[c2rust::src_loc = "154:1"]
pub unsafe extern "C" fn expr_ffi(mut opt: *mut pass_opt_t, mut ast: *mut ast_t) -> bool {
    let mut name: ast_ptr_t = 0 as *mut ast_t;
    let mut return_typeargs: ast_ptr_t = 0 as *mut ast_t;
    let mut args: ast_ptr_t = 0 as *mut ast_t;
    ...
    if !name.is_null() {
        ...
        ponyint_assert_fail(
            ...
            (*::core::mem::transmute::<&[u8; 9], &[libc::c_char; 9]>(b"expr_ffi\0")).as_ptr(),
        );
    };
    let mut decl: *mut ast_t = 0 as *mut ast_t;
    if !ffi_get_decl(&mut (*opt).check, ast, &mut decl, opt) {
        return 0 as libc::c_int != 0;
    }
    if decl.is_null() {
        ast_error(
            (*opt).check.errors,
            ast_child(ast),
            b"An FFI call needs a declaration\0" as *const u8 as *const libc::c_char,
        );
        return 0 as libc::c_int != 0;
    }
