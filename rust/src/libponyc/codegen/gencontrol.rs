...
unsafe extern "C" fn gen_then_clauses(
    ...
    while !parent.is_null() {
        match ast_id(parent) as libc::c_uint {
            ...
            88 | 90 | 89 => return,
            124 | 125 => {
                if ast_index(last) != (2 as libc::c_int as libc::c_ulong).try_into().unwrap() {
                    gen_expr(c, ast_childidx(parent, 2 as libc::c_int as usize));
                }
            }
            206 => {
                if ast_index(last) != (1 as libc::c_int as libc::c_ulong).try_into().unwrap() {
...
pub unsafe extern "C" fn gen_error(mut c: *mut compile_t, mut ast: *mut ast_t) -> LLVMValueRef {
    ...
    if !error_handler_expr.is_null() {
        match ast_id(error_handler_expr) as libc::c_uint {
            ...
            206 => {
                if !error_handler_expr.is_null() && clause == 0 {
                    gen_expr(
                        c,
...
pub unsafe extern "C" fn attach_branchweights_metadata(
    ...
    *fresh0 = LLVMMDStringInContext(
    ...
    let mut i: usize = 0;
    while i < (count as libc::c_ulong).try_into().unwrap() {
