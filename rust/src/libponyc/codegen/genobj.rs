...
pub mod TargetMachine_h {
    ...
    #[c2rust::src_loc = "64:9"]
    pub type LLVMCodeGenFileType = libc::c_uint;
    #[c2rust::src_loc = "66:5"]
...
pub use self::Core_h::{
    ...
    LLVMDisposeMessage, LLVMExternalLinkage, LLVMExternalWeakLinkage, LLVMFastCallConv,
    LLVMGHCCallConv, LLVMGhostLinkage, LLVMHHVMCCallConv, LLVMHHVMCallConv, LLVMHiPECallConv,
    ...
    LLVMPreserveMostCallConv, LLVMPrintModuleToFile, LLVMPrivateLinkage, LLVMSPIRFUNCCallConv,
    LLVMSPIRKERNELCallConv, LLVMSwiftCallConv, LLVMWeakAnyLinkage, LLVMWeakODRLinkage,
...
pub use self::TargetMachine_h::{
    LLVMAssemblyFile, LLVMCodeGenFileType, LLVMObjectFile, LLVMOpaqueTargetMachine,
...
#[no_mangle]
#[c2rust::src_loc = "4:1"]
pub unsafe extern "C" fn genobj(mut c: *mut compile_t) -> *const libc::c_char {
    let mut errors: *mut errors_t = (*(*c).opt).check.errors;
    if (*(*c).opt).limit as libc::c_uint == PASS_LLVM_IR as libc::c_int as libc::c_uint {
        let mut file_o: *const libc::c_char = suffix_filename(
            c,
            (*(*c).opt).output,
        ...
        if LLVMPrintModuleToFile((*c).module, file_o, &mut err) != 0 as libc::c_int {
            errorf(
                errors,
                0 as *const libc::c_char,
                b"couldn't write IR to %s: %s\0" as *const u8 as *const libc::c_char,
                file_o,
                err,
            );
    ...
    if (*(*c).opt).limit as libc::c_uint == PASS_BITCODE as libc::c_int as libc::c_uint {
        let mut file_o_0: *const libc::c_char = suffix_filename(
            c,
            (*(*c).opt).output,
            b"\0" as *const u8 as *const libc::c_char,
            (*c).filename,
        ...
        if LLVMWriteBitcodeToFile((*c).module, file_o_0) != 0 as libc::c_int {
            errorf(
                errors,
                0 as *const libc::c_char,
                b"couldn't write bitcode to %s\0" as *const u8 as *const libc::c_char,
                file_o_0,
            );
            return 0 as *const libc::c_char;
        }
    ...
    let mut fmt: LLVMCodeGenFileType = LLVMAssemblyFile;
    let mut file_o_1: *const libc::c_char = 0 as *const libc::c_char;
    if (*(*c).opt).limit as libc::c_uint == PASS_ASM as libc::c_int as libc::c_uint {
        fmt = LLVMAssemblyFile;
        file_o_1 = suffix_filename(
            c,
            (*(*c).opt).output,
            b"\0" as *const u8 as *const libc::c_char,
            (*c).filename,
    ...
    ) != 0 as libc::c_int
    {
        errorf(
            errors,
            0 as *const libc::c_char,
            b"couldn't create file: %s\0" as *const u8 as *const libc::c_char,
            err_0,
        );
        LLVMDisposeMessage(err_0);
        return 0 as *const libc::c_char;
    }
    return file_o_1;
}
