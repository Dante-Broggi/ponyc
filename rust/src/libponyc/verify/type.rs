...
#[c2rust::header_src = "/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/ast/id.h:1"]
pub mod id_h {
    extern "C" {
        #[c2rust::src_loc = "42:1"]
...
#[c2rust::src_loc = "5:1"]
unsafe extern "C" fn embed_struct_field_has_finaliser(
    mut opt: *mut pass_opt_t,
    mut field: *mut ast_t,
    mut base: *mut ast_t,
) -> bool {
    let mut type_0: *mut ast_t = ast_type(field);
    ...
    let mut final_0: *mut ast_t = ast_get(
        def,
    ...
    if !final_0.is_null() {
        ast_error(
            (*opt).check.errors,
            base,
            b"a struct cannot have a field with a _final method\0" as *const u8
                as *const libc::c_char,
        );
        if field != base {
            ast_error_continue(
                (*opt).check.errors,
                field,
                b"nested field is here\0" as *const u8 as *const libc::c_char,
            );
        }
        ast_error_continue(
            (*opt).check.errors,
            final_0,
            b"_final method is here\0" as *const u8 as *const libc::c_char,
        );
...
#[no_mangle]
#[c2rust::src_loc = "47:1"]
pub unsafe extern "C" fn verify_struct(mut opt: *mut pass_opt_t, mut ast: *mut ast_t) -> bool {
    if ast_id(ast) as libc::c_uint == TK_STRUCT as libc::c_int as libc::c_uint {
    } else {
        ponyint_assert_fail(
...
#[no_mangle]
#[c2rust::src_loc = "70:1"]
pub unsafe extern "C" fn verify_interface(mut opt: *mut pass_opt_t, mut ast: *mut ast_t) -> bool {
    if ast_id(ast) as libc::c_uint == TK_INTERFACE as libc::c_int as libc::c_uint {
    } else {
        ponyint_assert_fail(
    ...
    while !member.is_null() {
        match ast_id(member) as libc::c_uint {
            89 | 90 => {
                ...
                let mut children_0: [*mut *mut ast_t; 9] = [
                    &mut cap,
                    &mut id_0,
                    &mut type_params,
                    &mut params,
                ...
                if is_name_private(type_id_name) {
                    ast_error(
                        (*opt).check.errors,
                        id_0,
                        b"interfaces can't have private methods, only traits can\0" as *const u8
                            as *const libc::c_char,
                    );
                    ok = 0 as libc::c_int != 0;
                }
