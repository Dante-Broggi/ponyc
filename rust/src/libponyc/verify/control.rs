...
pub use self::ast_h::{
    ast_canerror, ast_cansend, ast_error, ast_get_children, ast_id, ast_mightsend, ast_ptr_t,
    ast_seterror, ast_setmightsend, ast_setsend,
};
...
#[no_mangle]
#[c2rust::src_loc = "4:1"]
pub unsafe extern "C" fn verify_try(mut opt: *mut pass_opt_t, mut ast: *mut ast_t) -> bool {
        ...
        || ast_id(ast) as libc::c_uint == TK_TRY_NO_CHECK as libc::c_int as libc::c_uint
    {
        ...
        ponyint_assert_fail(
            ...
            b"/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/verify/control.c\0"
                as *const u8 as *const libc::c_char,
            6 as libc::c_int as usize,
            (*::core::mem::transmute::<&[u8; 11], &[libc::c_char; 11]>(b"verify_try\0")).as_ptr(),
        );
    };
    ...
    if ast_id(ast) as libc::c_uint != TK_TRY_NO_CHECK as libc::c_int as libc::c_uint
        && !ast_canerror(body)
    {
        ast_error(
            (*opt).check.errors,
            body,
            b"try expression never results in an error\0" as *const u8 as *const libc::c_char,
        );
        return 0 as libc::c_int != 0;
    }
    if ast_canerror(else_clause) as libc::c_int != 0
        || ast_canerror(then_clause) as libc::c_int != 0
    {
        ast_seterror(ast);
    }
    if ast_cansend(body) as libc::c_int != 0
        || ast_cansend(else_clause) as libc::c_int != 0
        || ast_cansend(then_clause) as libc::c_int != 0
    {
        ast_setsend(ast);
    }
    if ast_mightsend(body) as libc::c_int != 0
        || ast_mightsend(else_clause) as libc::c_int != 0
        || ast_mightsend(then_clause) as libc::c_int != 0
    {
...
#[no_mangle]
#[c2rust::src_loc = "31:1"]
pub unsafe extern "C" fn verify_disposing_block(mut ast: *mut ast_t) -> bool {
    ...
    if ast_cansend(body) as libc::c_int != 0 || ast_cansend(dispose_clause) as libc::c_int != 0 {
        ast_setsend(ast);
    }
    if ast_mightsend(body) as libc::c_int != 0 || ast_mightsend(dispose_clause) as libc::c_int != 0
    {
        ast_setmightsend(ast);
    }
    return 1 as libc::c_int != 0;
