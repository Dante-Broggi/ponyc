...
use self::ast_h::{ast_error, ast_id, ast_name};
...
#[no_mangle]
#[c2rust::src_loc = "6:1"]
pub unsafe extern "C" fn check_id(
    mut opt: *mut pass_opt_t,
    mut id_node: *mut ast_t,
    ...
    if !id_node.is_null() {
        ...
        ponyint_assert_fail(
            b"id_node != NULL\0" as *const u8 as *const libc::c_char,
            b"/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/ast/id.c\0" as *const u8
                as *const libc::c_char,
            8 as libc::c_int as usize,
            (*::core::mem::transmute::<&[u8; 9], &[libc::c_char; 9]>(b"check_id\0")).as_ptr(),
        );
    };
    if ast_id(id_node) as libc::c_uint == TK_ID as libc::c_int as libc::c_uint {
    } else {
        ponyint_assert_fail(
            b"ast_id(id_node) == TK_ID\0" as *const u8 as *const libc::c_char,
            b"/Users/dantebroggi/Documents/GitHub/ponyc/src/libponyc/ast/id.c\0" as *const u8
                as *const libc::c_char,
    ...
    let mut prev: libc::c_char = '\0' as i32 as libc::c_char;
    if *name as libc::c_int == '$' as i32 {
        return 1 as libc::c_int != 0;
    }
    if *name as libc::c_int == '_' as i32 {
        name = name.offset(1);
        prev = '_' as i32 as libc::c_char;
        if *name as libc::c_int == '\0' as i32 {
            if spec & 0x20 as libc::c_int == 0 as libc::c_int {
                ast_error(
                    (*opt).check.errors,
                    id_node,
                    b"%s name cannot be \"%s\"\0" as *const u8 as *const libc::c_char,
                    desc,
                    ast_name(id_node),
                );
        ...
        if spec & 0x4 as libc::c_int == 0 as libc::c_int {
            ast_error(
                (*opt).check.errors,
                id_node,
                b"%s name \"%s\" cannot start with underscores\0" as *const u8
                    as *const libc::c_char,
                desc,
                ast_name(id_node),
            );
    ...
    if spec & 0x2 as libc::c_int != 0 as libc::c_int
        && ((*name as libc::c_int) < 'a' as i32 || *name as libc::c_int > 'z' as i32)
    {
        if spec & 0x4 as libc::c_int == 0 as libc::c_int {
            ast_error(
                (*opt).check.errors,
                id_node,
                b"%s name \"%s\" must start a-z\0" as *const u8 as *const libc::c_char,
                desc,
                ast_name(id_node),
            );
            return 0 as libc::c_int != 0;
            ...
            ast_error(
                (*opt).check.errors,
                id_node,
                b"%s name \"%s\" must start a-z or _(a-z)\0" as *const u8 as *const libc::c_char,
                desc,
                ast_name(id_node),
            );
            return 0 as libc::c_int != 0;
    ...
    if spec & 0x1 as libc::c_int != 0 as libc::c_int
        && ((*name as libc::c_int) < 'A' as i32 || *name as libc::c_int > 'Z' as i32)
    {
        if spec & 0x4 as libc::c_int == 0 as libc::c_int {
            ast_error(
    ...
    while *name as libc::c_int != '\0' as i32 && *name as libc::c_int != '\'' as i32 {
        if *name as libc::c_int == '_' as i32 {
            if spec & 0x8 as libc::c_int == 0 as libc::c_int {
                ast_error(
                    (*opt).check.errors,
                    id_node,
                    b"%s name \"%s\" cannot contain underscores\0" as *const u8
                        as *const libc::c_char,
                    desc,
                    ast_name(id_node),
                );
            ...
            if prev as libc::c_int == '_' as i32 {
                ast_error(
                    (*opt).check.errors,
                    id_node,
                    b"%s name \"%s\" cannot contain double underscores\0" as *const u8
                        as *const libc::c_char,
                    desc,
                    ast_name(id_node),
                );
    ...
    if prev as libc::c_int == '_' as i32 {
        ast_error(
            (*opt).check.errors,
            id_node,
            b"%s name \"%s\" cannot have a trailing underscore\0" as *const u8
                as *const libc::c_char,
            desc,
            ast_name(id_node),
        );
    ...
    if spec & 0x10 as libc::c_int == 0 as libc::c_int {
        ast_error(
            (*opt).check.errors,
            id_node,
            b"%s name \"%s\" cannot contain prime (')\0" as *const u8 as *const libc::c_char,
            desc,
            ast_name(id_node),
        );
        return 0 as libc::c_int != 0;
    ...
    while *name as libc::c_int != '\0' as i32 {
        if *name as libc::c_int != '\'' as i32 {
            ast_error(
                (*opt).check.errors,
                id_node,
                b"prime(') can only appear at the end of %s name \"%s\"\0" as *const u8
                    as *const libc::c_char,
                desc,
                ast_name(id_node),
            );
...
pub unsafe extern "C" fn check_id_type(
    mut opt: *mut pass_opt_t,
    mut id_node: *mut ast_t,
    mut entity_desc: *const libc::c_char,
) -> bool {
    return check_id(
        opt,
        id_node,
...
pub unsafe extern "C" fn check_id_type_param(
    mut opt: *mut pass_opt_t,
    mut id_node: *mut ast_t,
) -> bool {
    return check_id(
...
pub unsafe extern "C" fn check_id_package(
    mut opt: *mut pass_opt_t,
    mut id_node: *mut ast_t,
) -> bool {
    return check_id(
...
#[no_mangle]
#[c2rust::src_loc = "159:1"]
pub unsafe extern "C" fn check_id_field(mut opt: *mut pass_opt_t, mut id_node: *mut ast_t) -> bool {
    return check_id(
        opt,
        id_node,
...
pub unsafe extern "C" fn check_id_method(
    mut opt: *mut pass_opt_t,
    mut id_node: *mut ast_t,
) -> bool {
    return check_id(
...
pub unsafe extern "C" fn check_id_param(mut opt: *mut pass_opt_t, mut id_node: *mut ast_t) -> bool {
    return check_id(
        opt,
        id_node,
...
pub unsafe extern "C" fn check_id_local(mut opt: *mut pass_opt_t, mut id_node: *mut ast_t) -> bool {
    return check_id(
        opt,
        id_node,
        b"local variable\0" as *const u8 as *const libc::c_char,
        0x2 as libc::c_int | 0x8 as libc::c_int | 0x10 as libc::c_int | 0x20 as libc::c_int,
    );
}
#[no_mangle]
#[c2rust::src_loc = "187:1"]
pub unsafe extern "C" fn is_name_type(mut name: *const libc::c_char) -> bool {
    if *name as libc::c_int == '$' as i32 {
        name = name.offset(1);
    }
    if *name as libc::c_int == '_' as i32 {
...
pub unsafe extern "C" fn is_name_private(mut name: *const libc::c_char) -> bool {
    return *name.offset(0 as libc::c_int as isize) as libc::c_int == '_' as i32
        && *name.offset(1 as libc::c_int as isize) as libc::c_int != '\0' as i32
        || is_name_internal_test(name) as libc::c_int != 0
            && *name.offset(1 as libc::c_int as isize) as libc::c_int == '_' as i32;
}
#[no_mangle]
#[c2rust::src_loc = "204:1"]
pub unsafe extern "C" fn is_name_ffi(mut name: *const libc::c_char) -> bool {
    return *name.offset(0 as libc::c_int as isize) as libc::c_int == '@' as i32;
}
#[no_mangle]
#[c2rust::src_loc = "209:1"]
pub unsafe extern "C" fn is_name_internal_test(mut name: *const libc::c_char) -> bool {
    return *name.offset(0 as libc::c_int as isize) as libc::c_int == '$' as i32;
}
#[no_mangle]
#[c2rust::src_loc = "214:1"]
pub unsafe extern "C" fn is_name_dontcare(mut name: *const libc::c_char) -> bool {
    return *name.offset(0 as libc::c_int as isize) as libc::c_int == '_' as i32
        && *name.offset(1 as libc::c_int as isize) as libc::c_int == '\0' as i32;
}
